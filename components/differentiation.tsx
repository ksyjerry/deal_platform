...
export default function Differentiation() {
  ...
  const points = [
    ...
    },
    {
      title: "맞춤형 경험",
      description:
        "대기업, 금융기관, 스타트업까지 각 기업의 특성과 요구사항에 맞는 맞춤형 M&A 서비스를 제공합니다.",
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 p-8 text-white">
              ...
              <p className="text-base mb-4">
                전 세계 157개국 284,000명의 PwC 전문가 네트워크를 통한 글로벌
                수준의 M&A 서비스
              </p>
            ...
            <div className="space-y-8">
              {points.map((point, index) => (
                <motion.div
                  ...
                  variants={fadeIn}
                  className="bg-gray-50 px-4 py-3 rounded-lg border border-gray-100 transition-all duration-300 hover:shadow-md flex flex-row items-center justify-between min-h-[80px] text-left"
                >
                  {/* 왼쪽: 아이콘, 타이틀, stats */}
                  <div className="flex items-center gap-4 min-w-[220px]">
                    <div className="h-10 w-10 rounded-full bg-[#F4511E]/10 flex items-center justify-center flex-shrink-0">
                      {point.icon}
                    </div>
                    <div>
                      ...
                      <span className="text-xs font-bold text-[#F4511E] bg-[#F4511E]/10 px-2 py-0.5 rounded-full">
                        {point.stats}
                      </span>
                    </div>
                  </div>
                  {/* 오른쪽: description */}
                  <div className="flex-1 text-gray-600 text-sm pl-6">
                    {point.description}
                  </div>
                </motion.div>
              ))}
